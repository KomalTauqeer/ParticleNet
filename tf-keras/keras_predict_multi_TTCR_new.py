    ...
    if options.do_eval:
        ...
        for sample in ["TT"]:
            ...
            if sample == "TT": 
                ...
                print (true_labels)
                true_class = np.argmax(true_labels, axis=1).T.flatten()
                print (true_class)
            ...
            if sample == "TT":
                rootIO.add_fivebranches(file_path, treename, 'jetchargetagger_prob_nodeWp', 'F', np.array(PN_output[:,0]).flatten(), 'jetchargetagger_prob_nodeWn', 'F', np.array(PN_output[:,1]).flatten(),'jetchargetagger_prob_nodeZ', 'F', np.array(PN_output[:,2]).flatten(), 'jetchargetagger_ind', 'F', predicted_class.flatten(), 'jetchargetagger_trueind', 'F', true_class)
            else:
                rootIO.add_fourbranches(file_path, treename, 'jetchargetagger_prob_nodeWp', 'F', np.array(PN_output[:,0]).flatten(), 'jetchargetagger_prob_nodeWn', 'F', np.array(PN_output[:,1]).flatten(),'jetchargetagger_prob_nodeZ', 'F', np.array(PN_output[:,2]).flatten(), 'jetchargetagger_ind', 'F', predicted_class.flatten())
