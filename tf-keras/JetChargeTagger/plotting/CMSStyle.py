...
def setCMSEra(*eras,**kwargs):
  ...
  for era in eras:
    ...
    show_lumi = kwargs.get('show_lumi', True)
    if lumi:
      if string:
        string += ", "
      if show_lumi: string += "%s fb^{-1}"%(lumi) #Switch this off if lumi is not needed
    if cme:
      string += " (%s TeV)"%(cme)
...
def setCMSLumiStyle(pad, iPosX, **kwargs):
  ...
  #print "Value of outOfFrame: " + str(outOfFrame)
  if outOfFrame:
    ...
    latex.DrawLatex(l+0.06,1-t+lumiTextOffset*t,cmsText)
    if extraText:
      latex.SetTextFont(extraTextFont)
      latex.SetTextSize(extraTextSize*t)
      latex.SetTextAlign(align)
      latex.DrawLatex(posX+0.06,posY,extraText)
  elif drawLogo:
